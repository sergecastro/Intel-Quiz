...
type CelebrateOpts = { title?: string; sub?: string; flag?: string; };
...
function ensureStyles() {
  ...
  const css = `
  #celebration-overlay{position:fixed;inset:0;display:none;align-items:center;justify-content:center;background:rgba(10,12,16,.55);backdrop-filter:blur(6px);z-index:9999}
  #celebration-overlay.show{display:flex}
  #celebration-card{position:relative;width:min(620px,92vw);padding:28px 24px;border-radius:22px;background:linear-gradient(180deg,#1b2230,#141925);box-shadow:0 18px 60px rgba(0,0,0,.35), inset 0 0 0 2px rgba(255,255,255,.06);color:#eaf0ff;text-align:center}
  ...
  #celebration-close{display:inline-block;padding:12px 18px;border:0;border-radius:14px;cursor:pointer;background:linear-gradient(90deg,#33d17a,#64f3a4);color:#0b1320;font-weight:700;box-shadow:0 8px 24px rgba(50,230,150,.35);transition:transform .12s ease, filter .12s ease}
...
function ensureDom() {
  ...
  overlayEl.querySelector<HTMLButtonElement>('#celebration-close')!.addEventListener('click', hide);
